...
- **Double-tap sends Tab** — sends Tab when you double-tap empty terminal space. While the on-screen keyboard is open, swipe once to send an arrow key in that direction; keep holding for 400 ms to repeat every 45 ms, or move across the starting point to switch direction immediately. The start point and current swipe vector appear while the gesture is active. With the keyboard closed, the same swipe scrolls normally. Links, files, selections, pinching, and mouse gestures keep their existing behavior.
