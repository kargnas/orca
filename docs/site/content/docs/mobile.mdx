...
- Turn on **arrow key gestures** from the move button in the terminal accessory row when you'd rather drive the cursor than scroll. Swipe once anywhere on the terminal to send an arrow key in that direction, keep holding for 400 ms to repeat every 45 ms, or move across the starting point to switch direction immediately; the start point and current swipe vector appear while the gesture is active. A double-tap on empty terminal space sends Tab. The button starts every session in scroll mode, where touch scrolls, taps open links and files, long-press selects, and pinch zooms as usual — tap it again to go back. Both modes work on iOS and Android whether or not the on-screen keyboard is open.
...
- **Tap terminal to show keyboard** (Android) — on by default. Turn it off to keep the on-screen keyboard closed when tapping terminal output; explicit keyboard controls still open it.
- **Resize terminal for keyboard** (Android) — reduces terminal rows to the visible area while the on-screen keyboard is open. It is off by default, so terminal rows stay unchanged unless you enable it.
