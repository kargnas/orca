...
- **Double-tap sends Tab** — sends Tab when you double-tap empty terminal space. Move the second touch instead to send an arrow key in that direction; keep holding for 400 ms to repeat every 45 ms. A single swipe keeps the normal scroll behavior. Links, files, selections, pinching, and mouse gestures are excluded.
